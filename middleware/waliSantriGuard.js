...
const waliSantriGuard = async (req, res, next) => {
  try {
    if (!req.wali) {
    ...
    const rawSantriId = req.headers["x-santri-id"];
    ...
    const santriId = Number(rawSantriId);
    ...
    const allowed = req.wali.santri_ids.includes(santriId);

    if (!allowed) {
      const stillOwned = await waliAppService.ownsSantri(
        req.wali.nomor_hp,
        santriId,
        req.tenantId
      );
      ...
      req.wali.santri_ids.push(santriId);
    }
...
module.exports = waliSantriGuard;
